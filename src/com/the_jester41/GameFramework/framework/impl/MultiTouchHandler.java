...
public class MultiTouchHandler implements TouchHandler {
	static int SIZE = 20; 	
	boolean isTouched[] = new boolean[SIZE];
	int[] touchX = new int[SIZE];
	int[] touchY = new int[SIZE];
	...
	public synchronized boolean isTouchDown(int pointer) {
		return (pointer >= 0 && pointer <= 20)? isTouched[pointer] : false;
	}

	@Override
	public synchronized int getTouchX(int pointer) {
		return (pointer >= 0 && pointer <= 20)? touchX[pointer] : 0;
	}

	@Override
	public synchronized int getTouchY(int pointer) {
		return (pointer >= 0 && pointer <= 20)? touchY[pointer] : 0;
	}
